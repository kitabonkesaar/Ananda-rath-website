      ...
      <div className="relative h-72 md:h-96">
        {pkg.image_url ? (
          <img
            ...
            onError={(e) => {
              e.currentTarget.src =
                "https://placehold.co/1200x600/f1f5f9/94a3b8?text=Image+Not+Found";
            }}
          />
        ) : (
        ...
        <div className="grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex flex-wrap gap-4 rounded-2xl bg-card p-5 shadow-card">
              ...
              <div className="flex items-center gap-2 text-sm">
                <MapPin className="h-4 w-4 text-primary" /> {pkg.starting_from}
              </div>
              {pkg.is_full ? (
                <div className="flex items-center gap-2 text-sm font-bold text-destructive">
                  <Users className="h-4 w-4 text-destructive" /> 🚨 House Full / Sold Out
                </div>
              ) : (
                pkg.seats_left !== null &&
                pkg.seats_left > 0 && (
                  <div className="flex items-center gap-2 text-sm">
                    <Users className="h-4 w-4 text-accent" /> Only {pkg.seats_left} seats left!
                  </div>
                )
              )}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {pkg.inclusions.map((item) => (
                        <div
                          key={item}
                          className="flex items-center gap-3 rounded-xl bg-green-50 border border-green-100 p-3.5 transition-all hover:shadow-md hover:-translate-y-0.5"
                        >
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {pkg.exclusions.map((item) => (
                        <div
                          key={item}
                          className="flex items-center gap-3 rounded-xl bg-red-50 border border-red-100 p-3.5 transition-all hover:shadow-md hover:-translate-y-0.5"
                        >
